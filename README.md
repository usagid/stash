...
AES-256-GCM is the default. AES-128-GCM is available as an optional browser-side alternative; both provide authenticated encryption. Burnable stashes are atomically removed from SQLite on their first API read. Non-burnable stashes can only disappear through expiration cleanup—there is no public delete endpoint.

The SQLite implementation uses Node's built-in `node:sqlite`, so production requires Node.js 22.5 or newer. Set `STASH_DATABASE_PATH` to change the database location; it defaults to `.data/stashes.sqlite`.
